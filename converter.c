...
uint32_t byte_to_int(char *buffer) {
	...
	i = buffer[1] + buffer[2] * 256 + buffer[3] * 65536 + buffer[4] * 16777216;
...
void time_to_byte(struct timeval tv, char *buffer) {
    ...
    int_to_byte(buffer, sec);
    int_to_byte(buffer + 4, usec);

}

// Global Variable
ext_to_mimetype_entry_t ext_to_mimetype_list[MIMELISTLENGTH] = {
...
char *extension_to_mime_type(char *extension) {
	...
	for (i = 0; i <= MIMELISTLENGTH; i++) {
		if (strcmp(ext_to_mimetype_list[i].extension,extension) == 0) {
			return ext_to_mimetype_list[i].mimetype;
		}
	}
